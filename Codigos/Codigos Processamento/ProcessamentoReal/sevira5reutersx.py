...
if __name__ == "__main__":
    ...
    #1
    come_xuchu2 = pickle.load(open("../../../Objetos/Objetos Preprocessados Reuters/TF/3K/Lemma/arq_LSA.aug","rb"))
    if (not isinstance(come_xuchu2, np.ndarray)):
        come_xuchu2 = np.array(come_xuchu2.todense(), dtype=np.float64)
    #2
    come_xuchu3 = pickle.load(open("../../../Objetos/Objetos Preprocessados Reuters/TF/3K/Lemma/arq.aug","rb"))
    if (not isinstance(come_xuchu3, np.ndarray)):
        come_xuchu3 = np.array(come_xuchu3.todense(), dtype=np.float64)
    #3
    come_xuchu1 = pickle.load(open("../../../Objetos/Objetos Preprocessados Reuters/TF/3K/Normal/arq_LSA.aug","rb"))
    if (not isinstance(come_xuchu1, np.ndarray)):
        come_xuchu1 = np.array(come_xuchu1.todense(), dtype=np.float64)
    
    top_3 = [(True, 'media', 'reuters', 'manhattan', 'x', 'Normal', 'TF', '3k', come_xuchu1),(True, 'media', 'reuters', 'euclidiana', 'x', 'Lemma', 'TF', '3k', come_xuchu2), (False, 'media', 'reuters', 'cosseno', 'x', 'Lemma', 'TF', '3k', come_xuchu3)]
