...
if __name__ == '__main__':
    ...
    # 4
    come_xuchu4 = pickle.load(open("../../../Objetos/Objetos Preprocessados Reuters/TFIDF/3k/Normal/arq_LSA.aug", "rb"))
    if (not isinstance(come_xuchu4, np.ndarray)):
        come_xuchu4 = np.array(come_xuchu4.todense(), dtype=np.float64)
        come_xuchu6 = come_xuchu4.copy()
    # 5
    ...
    top_5 = [
        (True, 'linear', 'reuters', 10, 0.1, 5, 3, 'Normal', 'linear', 'TFIDF', '3k', come_xuchu4, 'bubble'),
        (True, 'exponential', 'reuters', 10, 0.7, 5, 2, 'Lemma', 'linear', 'TFIDF', '3k', come_xuchu5, 'bubble'),
        (True, 'linear', 'reuters', 15, 0.7, 5, 2, 'Normal', 'linear', 'TFIDF', '3k', come_xuchu6, 'bubble')]
    resposta = []
    ...
    for i, melhor in enumerate(top_5):
        ...
        for j in range(5):
            ...
            som.train(data=dados, epochs=1000, radius0=melhor[6], radiusN=1, radiuscooling=melhor[8], scale0=melhor[4],
                      scaleN=0.01, scalecooling=melhor[1])
