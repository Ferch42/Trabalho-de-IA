...
def distancias_clusterz(dados,centroid,distancia_selecionada2):
    
    respp=[]
    for dado in dados:
        respp.append(distancia_selecionada2(centroid,dado))
...
class ultra_omega_alpha_kmeans:
    ...
    def __init__(self, no_clusters = 2, inicializacao = 'padrao', distancia = "euclidiana", algoritmo = 'media',no_iteracoes = 500,n_jobs=-1):
    ...
    def calcula_distancia(self, distancia_selecionada): #O(n*c)
        ...
        distancias_clusters=Parallel(n_jobs=self.n_jobs,  backend="threading")(delayed(distancias_clusterz)(self.dados,centroid,distancia_selecionada) for centroid in self.centroids)
