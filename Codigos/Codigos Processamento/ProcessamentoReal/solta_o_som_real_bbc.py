...
if __name__ == '__main__':
    ...
    come_xuchu1 = pickle.load(open("../../../Objetos/ObjetosPreProcessados/TF/3K/Normal/tfVector3k.aug","rb"))
    if (not isinstance(come_xuchu1, np.ndarray)):
        come_xuchu1 = np.array(come_xuchu1.todense(), dtype=np.float64)
    #2
    come_xuchu2 = pickle.load(open("../../../Objetos/ObjetosPreProcessados/TFIDF/3K/Lemma/tfidfVector3kLemmatizerLSA.aug","rb"))
    if (not isinstance(come_xuchu2, np.ndarray)):
        come_xuchu2 = np.array(come_xuchu2.todense(), dtype=np.float64)

    top_5 = [(False, 'exponential','bbc', 15, 0.1, 2, 5, 'Normal', 'linear', 'TF', '3k', come_xuchu1, 'gaussian'), (True, 'exponential', 'bbc', 10, 0.4, 2, 2, 'Lemma', 'linear', 'TFIDF', '3k', come_xuchu2, 'bubble'), (False, 'exponential', 'bbc', 15, 0.4, 2, 2, 'Normal', 'linear', 'TF', '3k', come_xuchu1, 'gaussian'), (False, 'linear', 'bbc', 15, 0.4, 2, 2, 'Normal', 'linear', 'TF', '3k', come_xuchu1, 'gaussian'), (False, 'exponential', 'bbc', 20, 0.4, 2, 7, 'Normal', 'linear', 'TF', '3k', come_xuchu1, 'gaussian')]
    resposta = []

    for i,melhor in enumerate(top_5):
        print(str(i+1) + "ª melhor configuracao")
        som = somoclu.Somoclu(melho[3], melhor[3], neighborhood=melhor[12])
